...
@Component({
  selector: 'app-TransEditModal',
  templateUrl: './TransEditModal.component.html',
  styleUrls: ['./TransEditModal.component.css']
})
export class TransEditModalComponent implements OnInit {
  ...
  getCurrencies() {
    this.projectService.getCurrencies().subscribe(
      response => {
        ...
        currency => currency.currencyName === this.trans.currencyName
      )[0];
        this.transService.formEditModalTrans
        .get('CurrencyId')
